...
{
	// 步骤1: 通过inode中i_cdev指针反向找到nvme_ctrl指针
	struct nvme_ctrl *ctrl =
...
static int nvme_dev_user_cmd(struct nvme_ctrl *ctrl, void __user *argp)
{
	...
	// 检查是不是只有一个namespace，如果有多个的话不支持IO操作
...
static int __init nvme_core_init(void)
{
	...
	// 步骤2: 申请集中任务队列 nvme-wq用于处理NVMe设备请求，nvme-reset-wq用于设备复位等
	...
	// 步骤3: 分配设备号 函数会在内核中查找一个未被使用的字符设备号，并将其分配给调用者使用。
	result = alloc_chrdev_region(&nvme_chr_devt, 0, NVME_MINORS, "nvme");
	if (result < 0)
		goto destroy_delete_wq;

	// 步骤4: 在Linux内核中创建设备类（device class), class_create中会进行注册
	nvme_class = class_create(THIS_MODULE, "nvme");
	if (IS_ERR(nvme_class)) {
		result = PTR_ERR(nvme_class);
		goto unregister_chrdev;
	}
	nvme_class->dev_uevent = nvme_class_uevent; // 步骤5: 指定UEVENT事件处理函数
